...
import { MatmulTiled, matmulTiledWorkgroups } from '../../src/kernels/matmul_tiled.ts';
...
async function run(shape: MatmulShape, a: Float32Array, b: Float32Array): Promise<Float32Array> {
  ...
  const matmul = new MatmulTiled(ctx.device, cache);
...
describe('matmul_tiled', () => {
  ...
  it('reuses one pipeline across instances', async () => {
    ...
    const first = new MatmulTiled(ctx.device, cache);
    const second = new MatmulTiled(ctx.device, cache);
  ...
  it('derives dispatch geometry that covers the output exactly once', () => {
    expect(matmulTiledWorkgroups({ m: 32, n: 32, k: 1 })).toEqual([2, 2, 1]);
    expect(matmulTiledWorkgroups({ m: 17, n: 129, k: 1 })).toEqual([9, 2, 1]);
    expect(matmulTiledWorkgroups({ m: 1, n: 1, k: 1 })).toEqual([1, 1, 1]);
