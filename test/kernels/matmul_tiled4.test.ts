...
import { MatmulTiled4, matmulTiled4Workgroups } from '../../src/kernels/matmul_tiled4.ts';
...
async function run(shape: MatmulShape, a: Float32Array, b: Float32Array): Promise<Float32Array> {
  ...
  const matmul = new MatmulTiled4(ctx.device, cache);
...
describe('matmul_tiled4', () => {
  ...
  const shapes: Array<[string, MatmulShape]> = [
    ...
    // Specific to the 64-row footprint: one row past it, so the second workgroup row has
    // 63 of its 64 rows out of range and every accumulator mask gets exercised.
    ['65x33x40 — one row past the 64-row tile', { m: 65, n: 33, k: 40 }],
    ['80x16x16 — rows straddle the footprint edge', { m: 80, n: 16, k: 16 }],
  ];
  ...
  it('does not write past the last output element', async () => {
    // 17x17 dispatches a single workgroup whose footprint is 64x16, so it produces 1024
    // candidate outputs for 289 real ones — 735 masked writes, spread across all four
  ...
  it('reuses one pipeline across instances', async () => {
    ...
    const first = new MatmulTiled4(ctx.device, cache);
    const second = new MatmulTiled4(ctx.device, cache);
  ...
  it('derives dispatch geometry that covers the output exactly once', () => {
    expect(matmulTiled4Workgroups({ m: 32, n: 32, k: 1 })).toEqual([2, 1, 1]);
    expect(matmulTiled4Workgroups({ m: 17, n: 129, k: 1 })).toEqual([9, 1, 1]);
    expect(matmulTiled4Workgroups({ m: 1, n: 1, k: 1 })).toEqual([1, 1, 1]);
    expect(matmulTiled4Workgroups({ m: 64, n: 16, k: 1 })).toEqual([1, 1, 1]);
    expect(matmulTiled4Workgroups({ m: 65, n: 16, k: 1 })).toEqual([1, 2, 1]);
    expect(matmulTiled4Workgroups({ m: 256, n: 64, k: 1 })).toEqual([4, 4, 1]);
