...
import { MatmulTiled8, matmulTiled8Workgroups } from '../../src/kernels/matmul_tiled8.ts';
...
async function run(shape: MatmulShape, a: Float32Array, b: Float32Array): Promise<Float32Array> {
  ...
  const matmul = new MatmulTiled8(ctx.device, cache);
...
describe('matmul_tiled8', () => {
  ...
  const shapes: Array<[string, MatmulShape]> = [
    ...
    // Specific to the 128-row footprint. A thread's eight rows span 113 rows of C, so these
    // leave workgroups with almost every mix of in-range and out-of-range accumulators.
    ['65x33x40 — inside one 128-row tile, ragged', { m: 65, n: 33, k: 40 }],
    ['129x33x40 — one row past the 128-row tile', { m: 129, n: 33, k: 40 }],
    ['144x16x16 — rows straddle the footprint edge', { m: 144, n: 16, k: 16 }],
    ['113x17x33 — every accumulator mask mixed', { m: 113, n: 17, k: 33 }],
  ];
  ...
  it('does not write past the last output element', async () => {
    // 17x17 dispatches a single workgroup whose footprint is 128x16, so it produces 2048
    // candidate outputs for 289 real ones — 1759 masked writes, spread across all eight
    // accumulators. The output buffer is bound large enough that a missing guard would land
  ...
  it('reuses one pipeline across instances', async () => {
    ...
    const first = new MatmulTiled8(ctx.device, cache);
    const second = new MatmulTiled8(ctx.device, cache);
  ...
  it('derives dispatch geometry that covers the output exactly once', () => {
    expect(matmulTiled8Workgroups({ m: 32, n: 32, k: 1 })).toEqual([2, 1, 1]);
    expect(matmulTiled8Workgroups({ m: 17, n: 129, k: 1 })).toEqual([9, 1, 1]);
    expect(matmulTiled8Workgroups({ m: 1, n: 1, k: 1 })).toEqual([1, 1, 1]);
    expect(matmulTiled8Workgroups({ m: 65, n: 16, k: 1 })).toEqual([1, 1, 1]);
    expect(matmulTiled8Workgroups({ m: 128, n: 16, k: 1 })).toEqual([1, 1, 1]);
    expect(matmulTiled8Workgroups({ m: 129, n: 16, k: 1 })).toEqual([1, 2, 1]);
    expect(matmulTiled8Workgroups({ m: 256, n: 64, k: 1 })).toEqual([4, 2, 1]);
